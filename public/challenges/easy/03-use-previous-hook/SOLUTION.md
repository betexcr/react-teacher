...
- `onClick={() => setN((v) => v + 1)}` — **onClick** — In "usePrevious Hook", clicking updates state via . On render N, ref still holds value from render N-1 because the effect has not run yet. After paint, effect sets ref to the current value for next time.
