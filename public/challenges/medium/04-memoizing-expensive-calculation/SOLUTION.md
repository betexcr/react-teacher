...
- `onClick={() => setUnrelated((u) => u + 1)}` — **onClick** — In "Memoizing Expensive Calculation", clicking updates state via . Bumping unrelated counter does not re-run fib because deps unchanged.
