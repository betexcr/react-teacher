...
- `onClick={() => passwordRef.current?.focus()}` — **onClick** — In "Focus an Input with useRef", this runs when the user clicks this button. Buttons call focus on demand without re-render.
- `onClick={() => emailRef.current?.focus()}` — **onClick** — In "Focus an Input with useRef", this runs when the user clicks this button. Buttons call focus on demand without re-render.
