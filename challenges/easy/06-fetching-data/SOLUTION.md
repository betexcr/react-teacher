...
- `onClick={() => load()}` — **onClick** — In "Fetching Data", this runs when the user clicks this button. load centralizes fetch logic. Effect creates AbortController per mount. Refetch calls load without aborting unless you add that.
