...
- `onClick={() => setPage((p) => p - 1)}` — **decrement click** — In "Pagination Implementation", this subtracts 1 using the latest state. Effect refetches when page changes; buttons disabled at edges.
- `onClick={() => setPage((p) => p + 1)}` — **increment click** — In "Pagination Implementation", this adds 1 using the latest state. Effect refetches when page changes; buttons disabled at edges.
