...
const decks = [
  ...
  },
  {
    ...
    cards: [
      ...
      { front: 'React.FC vs function signature?', back: 'Prefer explicit props type on function; React.FC adds implicit children and is less common now.' },
      ...
      { front: 'forwardRef generics', back: 'forwardRef<HTMLButtonElement, ButtonProps>((props, ref) => ...)' },
  ...
  },
  {
    ...
    title: 'React 19',
    cards: [
      { front: 'Actions (React 19)', back: 'Functions passed to form action or useTransition that handle async work; useActionState manages pending/error/result.' },
      { front: 'useActionState', back: 'Hook for action state: [state, dispatch, isPending]. Replaces some useFormState patterns.' },
      ...
      { front: 'Improved hydration errors', back: 'Clearer mismatch diffs; recoverable errors in some cases without full client re-render.' },
      { front: 'Server Components default in frameworks', back: 'RSC ship zero JS for server-only components; "use client" marks client boundary.' },
      ...
      { front: 'Batching improvements', back: 'More automatic batching across async boundaries in 18+; 19 continues predictable updates.' },
      ...
      { front: 'Activity / Offscreen (experimental)', back: 'Patterns to hide UI while preserving state; check current React docs for status.' },
    ],
  },
  {
    ...
    cards: [
      ...
      { front: 'Class componentDidMount equivalent?', back: 'useEffect(() => { ... }, []) — empty deps run once after mount.' },
      { front: 'getDerivedStateFromProps equivalent?', back: 'Adjust state during render when props change: setState when prop !== state, or derive during render.' },
      { front: 'shouldComponentUpdate equivalent?', back: 'React.memo for components; useMemo for expensive values.' },
      { front: 'componentWillUnmount equivalent?', back: 'Effect cleanup function.' },
      { front: 'Why no componentWillMount?', back: 'Unsafe for SSR and async; replaced by constructor/render patterns and effects.' },
  ...
  },
  {
    ...
    cards: [
      ...
      { front: 'Profiler API', back: '<Profiler onRender> measures commit time in dev to find slow subtrees.' },
      ...
      { front: 'Server Components perf', back: 'Zero client JS for server-only UI; fetch on server close to data.' },
  ...
  },
  {
    ...
    cards: [
      { front: 'React Router v6 basics', back: 'BrowserRouter, Routes, Route path element, Link, NavLink, useNavigate.' },
      ...
      { front: 'Next.js App Router', back: 'File-system routes; layouts.js; server components by default.' },
  ...
  },
  {
    ...
    cards: [
      ...
      { front: 'staleTime vs cacheTime', back: 'staleTime: data fresh period; gcTime: unused cache retention before garbage collect.' },
      ...
      { front: 'Suspense for data', back: 'Throw promise in render; Suspense boundary shows fallback until resolved.' },
  ...
  },
  {
    ...
    cards: [
      ...
      { front: 'Server Actions forms', back: 'React 19 form action prop; progressive enhancement without client JS.' },
  ...
  },
  {
    ...
    cards: [
      { front: 'Server Component', back: 'Runs on server only; can async fetch; not shipped to client bundle (no hooks/state).' },
      ...
      { front: 'Hydration', back: 'Client attaches event listeners to server HTML; mismatch causes errors.' },
      ...
      { front: 'Caching in Next', back: 'fetch cache, revalidate, unstable_cache, dynamic = force-dynamic.' },
      { front: 'SEO benefit', back: 'SSR/SSG deliver crawlable HTML; CSR-only hurts SEO for content pages.' },
      ...
      { front: 'Partial Prerendering', back: 'Static shell + streamed dynamic holes (Next 15+ PPR).' },
  ...
  },
  {
    ...
    cards: [
      { front: 'State libraries comparison', back: 'Redux: predictable global. Zustand: minimal. Jotai/Recoil: atoms. Query: server cache.' },
      ...
      { front: 'React Native bridge', back: 'Share business logic; platform-specific UI components.' },
  ...
  },
  {
    ...
    cards: [
      ...
      { front: 'layout.tsx', back: 'Shared UI wrapping segments; persists state across navigations within segment.' },
      ...
      { front: 'Middleware', back: 'middleware.ts runs on edge before request; auth, redirects, headers.' },
  ...
  },
  {
    ...
    cards: [
      ...
      { front: 'Offscreen / Activity', back: 'Deprioritize hidden UI preserving state (evolving APIs).' },
  ...
  },
  {
    ...
    cards: [
      ...
      { front: 'Purge/content config', back: 'Tailwind scans src for class names; wrong paths bloat or strip needed.' },
  ...
  },
  {
    ...
    cards: [
      ...
      { front: 'useMemo / useCallback', back: 'Performance optimization; cache values/functions when deps stable.' },
      ...
      { front: 'useImperativeHandle', back: 'Customize ref value exposed to parent with forwardRef.' },
...
const processedDecks = decks.map((d) => ({
  ...
  cards:
    d.id === 'react-fundamentals'
      ? fundamentalsCards
      : d.cards.map((c) => enrichCard(d.title, c.front, c.back)),
}));
